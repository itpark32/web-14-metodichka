...
!!! abstract "Фокус"
    - **Нужно знать:** точка входа, граф зависимостей, режим разработки и production.
    - **Часто в работе:** npm-скрипты, source maps, ESLint и Prettier.
    - **Достаточно узнавать:** ручная настройка сложных loaders и plugins.
...
> Сборщик начинает с точки входа, строит граф зависимостей и формирует файлы для браузера. ESLint проверяет правила и вероятные ошибки, Prettier отвечает за единое форматирование. Это разные инструменты, которые дополняют друг друга.
