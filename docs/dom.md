...
!!! abstract "Фокус"
    - **Нужно знать:** поиск элемента, `classList`, `textContent`, создание и удаление узлов.
    - **Часто в работе:** `closest`, `dataset`, `DocumentFragment`.
    - **Достаточно узнавать:** живые HTMLCollection и редкие обходы дерева.
...
> DOM является объектным представлением HTML. `querySelector` возвращает первый подходящий элемент или `null`, `querySelectorAll` возвращает статический список. Для текста использую `textContent`, классы меняю через `classList`, а пользовательские строки не вставляю через `innerHTML` без очистки.
