...
!!! abstract "Фокус"
    - **Нужно знать:** `addEventListener`, всплытие, `target`, `currentTarget`, `submit`.
    - **Часто в работе:** делегирование, `preventDefault`, `FormData`.
    - **Достаточно узнавать:** фаза перехвата и `stopImmediatePropagation`.
...
> События обычно всплывают от целевого элемента к предкам. `target` показывает, где событие началось, `currentTarget` указывает элемент текущего обработчика. Делегирование ставит один обработчик на предка. У формы слушаю `submit`, чтобы поддержать и кнопку, и Enter.
...
[Следующая тема: BOM и хранилища](bom-storage.md)
